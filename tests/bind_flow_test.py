...
commands._handle_bind 与 web.api_bind_cb，验证整条链路（跨适配器路径下走通用
bot.send / compat 辅助）。
"""
...
class FakeBot:
    """记录调用的假 bot：send/send_group_msg/send_private_msg/delete_msg。"""

    def __init__(self) -> None:
        self.calls: list[tuple[str, dict]] = []
        self.self_id = "10001"
        self.type = "OneBot V11"

    async def send(self, event=None, message=None, **kwargs) -> dict:
...
async def test_bind_flow_in_group(monkeypatch):
    ...
    bot = FakeBot()
    monkeypatch.setattr("nonebot_plugin_azurlane.compat.resolve_bot", lambda self_id=None: bot)
    ...
    # 2) msg_id 已补记进会话，且记录了 self_id / 场景
